...
const Navbar = () => {
  ...
  return (
    <div className={`fixed z-20 flex ${changeNavbarColor ? 'bg-orange backdrop-blur-xl' : 'bg-transparent'} items-center justify-between w-full px-5 py-4 overflow-visible xl:py-0`}>
      ...
      <div className='hidden sm:block'>
        <div className='flex items-center text-white font-roboto'> 
          <Link className="relative px-4 py-4 group">
            <p>Home</p>
            <div className="absolute bottom-0 left-0 w-0 h-1 transition-all duration-300 bg-yellow group-hover:w-full"></div>
          </Link>
          <Link className="relative px-4 py-4 group">
            <p>Jadwal</p>
            <div className="absolute bottom-0 left-0 w-0 h-1 transition-all duration-300 bg-yellow group-hover:w-full"></div>
